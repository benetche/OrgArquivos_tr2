...
void parseDescLine(lineFile *lf, char *line) {
    ...
    setLineHeader(lf, descreveCodigo, descreveCartao, descreveNome,
                  descreveCor);
}
...
lineRecord *parseLineFromStrLine(char *line) {
    ...
    char *corLinha =
        safeToDynamic(strtok_single(rest, ",", &rest), &tamanhoCor);

    return newLineRecord(removido, codLinha, aceitaCartao, tamanhoNome,
                         nomeLinha, tamanhoCor, corLinha);
}
...
void parseCSVToLines(lineFile *lf, char *filename) {
    ...
    lf->records = (lineRecord **)realloc(lf->records, ((lf->nRecords)) *
                                                          sizeof(lineRecord *));
